...
V Role-Based Access Control (RBAC), je přístup ke zdrojům na základě založen na \uv{roli kterou uživatel hraje}. V tomto modelu, správce přiřadí uživateli roli, která má předem pridělena určitá práva a výsady. Vzhledem k~uživateli přidělené rolí, může uživatel přístupova k~určitým zdrojům a plnit specifické úkolů. RBAC je také známý jako Non-Discretionary Access Control. Role přiřazené uživatelům, jsou centrálně spravovány.
