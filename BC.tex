...
nyní je třeba spustit script start6, který vytvoří potřebné složky a~virtuální souborové systému. Skript také provede přihlášení do systému prostřednictvím prostředí chroot.
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
...
Ihned po balíčku glibc je spuštěn script chap6/locales, který kromě testu jazyků nastavuje i~časovou zónu. Je třeba otevřít nové terminálové okno, přihlásit se do systému pomocí skriptu:
    \begin{codeframe} 
\begin{Verbatim}{frame=single}
...
\begin{itemize}
 ...
 \item Posledním krokem ke kterému scripty navedou je pak nastavení programu grub. Je třeba nastavit /boot/grub/grub.cfg dle potřeb systému a~uživatele.
\end{itemize}

Nyní je již třeba postupovat dle knihy, otestovat nastavení programu GRUB a~aktualizovat Master Boot Record. Tyto kroky však nejsou třeba pokud se chystá uživatel provozovat LFS vedle jiného systému, v~tom případě není třeba nastavovat ani soubor /boot/grub/grub.cfg a~stačí pouze přidat položku LFS do souboru nastavení programu GRUB ve stávajícím systému.
