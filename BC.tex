...
\begin{description}
 ...
 \end{description}\label{sec:sloz}
    \begin{codeframe} 
      \begin{Verbatim}{frame=single}
 {
...
\subsubsection{Kulaté a složené závorky}
Složené závorky se používají nejen pro oddělení jména proměnné nebo pole od dalších znaků v~rámci příkazu~\odkazNaKapitolu{sec:pole}. Dokážou také seskupovat několik příkazů do~jednoho celku. To se může hodit hned v~několika případech: když chceme použít konstrukci, která umí vykonat pouze jeden element, chceme-li najednou přesměrovat výstup z~několika příkazů~\odkazNaKapitolu{sec:sloz} nebo~třeba pokud chceme nechat celou skupinu úloh provést na~pozadí.~\cite{BASHcz}\\
...
\end{Verbatim} 
    \end{codeframe}
Středník za posledním příkazem je zde nutný, podobně jako mezera za~otevírací a~před~zavírací závorkou. Můžeme použít také závorky kulaté, které mezery ani~závěrečný středník nepotřebují.~\cite{BASHcz}\\ Příkaz by pak mohl vypadat takto:
    \begin{codeframe} 
      \begin{Verbatim}{frame=single}
(sleep 10 ; echo Budíček!) &
\end{Verbatim} 
...
Vedle způsobu zápisu se konstrukce s~kulatými a složenými závorkami liší ještě v~jednom – předávání hodnot proměnných prostředí. To, co uzavřeme do~složených závorek, poběží stále v~aktuálním Bashi, takže po~skončení těchto operací budou provedené změny stále platit.~\cite{BASHcz} 
...
\end{Verbatim} 
    \end{codeframe}
Na~první echo (to v~závorkách) by měla být reakce 3, protože~jsme v~závorkách hodnotu proměnné změnili z~5 na~3. Na~druhé echo (za~závorkami) bude odpověď zase 3, protože příkazy provedené v~závorkách jsou platné pro aktuální shell. Jinak tomu samozřejmě bude v~případě závorek kulatých.~\cite{BASHcz}
    \begin{codeframe} 
      \begin{Verbatim}{frame=single}
$ husy="5"
$ (husy="3" ; echo $husy)
...
\end{Verbatim} 
    \end{codeframe}
Ukládat položky pole do proměnných lze také velmi jednoduše:\label{sec:pole}
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
...
\end{array}$
\end{center}
\caption[Webové rozhraní s nastavením SUSE Studio]{Webové rozhraní s nastavením systému, vlevo výběr základního rozhraní, vpravo podrobnější nastavení}
\end{figure}
...
\end{array}$
\end{center}
\caption[Webové rozhraní Instalinux, aplikace/uživatelé]{Webové rozhraní Instalinux, vlevo výběr aplikací, vpravo nastavení uživatelů}
\end{figure}
\newpage
...
\end{array}$
\end{center}
\caption[Webové rozhraní s nastavením NimbleX, základní nastavení/office]{Webové rozhraní s nastavením NimbleX, vlevo výběr míry nastavení, vpravo nastavení programů v sekci office}
\end{figure}
\begin{figure}[h]
\begin{center}$
\begin{array}{cc}
...
\end{array}$
\end{center}
\caption[Webové rozhraní s nastavením NimbleX, wallpaper/zvuk]{Webové rozhraní s nastavením NimbleX, vlevo výběr wallpaperu, vpravo nastavení zvuku}
\end{figure}
...
\b{POZOR!!! takový přístup se nepoužívá! Uživatelé bez hesla se používají pouze na LiveCD!}
 \item[Uživatel bez přihlášení] uživatel bez přihlášení se používá pro služby (v~Linuxu daemony, česky někdy též démony), je to speciální druh uživatele, za kterého se nelze přihlásit a~používá se aby byla práva služeb oddělena od uživatelů a~neběžela pod oprávnění uživatele root. Jejich nastavení se liší od normálního uživatele pouze v~tom, že~v~souboru \textit{/etc/passwd} je defaultní shell jako \textit{/bin/bash} nahrazen \textit{/bin/nologin} nebo \textit{/bin/false}\\
\textit{/etc/shadow}
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
...
\end{Verbatim} 
    \end{codeframe}
Skript je přerušen pro opětovné přihlášení a~je potřeba ho spustit znovu.\\
...
Po sestavení několik dalších balíčků je skript \textit{chroot\_makeall} ukončen a~je~možnost, stejně jako na~konci kapitoly 5, zmenšit velikost systému smazání debugovacích symbolů. K tomuto je třeba se přihlásit do systému a~spustit skript pro smazání debugovacích symbolů.
    \begin{codeframe} 
\begin{Verbatim}{frame=single}
...
Posledním krokem je pak opět v~chrootu LFS nahrát MBR (Master boot record) na~disk s~LFS. Je~třeba nastavit diskovou adresu v \textit{/dev} např. \textit{/dev/sda /dev/sdb}, opět bez čísla oddílu.
    \begin{codeframe} 
\begin{Verbatim}{frame=single}
...
\end{Verbatim} 
    \end{codeframe}
Po instalaci wget je už možné přímo z~LFS nainstalovat Xserver. Pokud je třeba změnit nějaké proměnné, např. proměnou WAIT, která určuje zda se má skript pozastavit v~každém kroku, je možné toto provést v~souboru \textit{/scripts/BLFS/variables}.
    \begin{codeframe} 
\begin{Verbatim}{frame=single}
./scripts/BLFS/xorg
\end{Verbatim} 
    \end{codeframe}
Skript končí naběhnutím testu Xserveru a~vytvořením konfiguračního souboru \textit{xorg.conf}. Je třeba přesunout konfigurační soubor do \textit{/etc/X11}, vytvořit soubor \textit{.xinitrc}, který říká Xserveru jaké aplikace při startu spustit a~vytvořit složku pro log soubor.
...
\end{Verbatim} 
    \end{codeframe}
Taková chyba je způsobena nastavením driveru Xserveru je třeba v~souboru \textit{/etc/X11/xorg.conf} změnit v sekci Device položku Driver z~hodnoty fbdev na vesa.
    \begin{codeframe} 
\begin{Verbatim}{frame=single}
Section "Device"
  ....
...
Na DVD dostupné ve složce \it{BC.TvorbaLinuxoveDistribuce}. BC.pdf je výstupem ve~formátu PDF. BC.tex je zdrojovým kódem v~jazyce \LaTeX. A~ve~složce \it{sablona} se~nachází soubor \it{sablona.tex} jež je šablonou pro FAI UTB.

\priloha{Zdrojové kódy LFS by BASH scripts}

Zdrojové kódy pro automatické sestavení distribuce LFS.\\

Dostupné na DVD (složka \it{LFS---by-bash-scripts}) na~portále stag a~na~internetu na~adrese\\

\href{https://github.com/zajca/LFS---by-bash-scripts}{https://github.com/zajca/LFS---by-bash-scripts}\\

Návod na použití skriptů \odkazNaKapitolu{sec:sestaveni} nebo anglicky v~README souboru projektu.\\

Na portále STAG a~na DVD dostupná verze BCv6.8. Verze dostupná na~internetu se~může lišit, návod jak se dostat k~verzi BCv6.8 \odkazNaKapitolu{navod:github}

\priloha{Funkční toolchain pro 64bit systém}
...
\priloha{Funkční sestavení distribuce LFS včetně X-serveru}

Dostupné pouze na DVD jako soubor LFS.ova (2,573,794,816 bytes)\\

Jedná se o~již nastavený virtuální počítač pro zdarma dostupný program VirtualBox. Stačí pouze otevřít VirtualBox a~spustit \it{FILE -> Import Appliance}. Velmi jednoduchý wizard pak nahraje vše potřebné a~přidá položku LFS.\\
...
Spuštění grafického serveru příkazem \it{xinit}.
