...
V Role-Based Access Control (RBAC), je přístup ke zdrojům založen na \uv{roli kterou uživatel hraje}. V tomto modelu, správce přiřadí uživateli roli, která má předem přidělena určitá práva a~výsady. Vzhledem k~uživateli přidělené rolí, může uživatel přístupová k~určitým zdrojům a~plnit specifické úkoly. RBAC je také známý jako Non-Discretionary Access Control. Role přiřazené uživatelům, jsou centrálně spravovány.
...
SSH znamená, ale~kromě snadného přístupu uživateli do vzdáleného počítače i~cestu pro nějakého útočníka do počítače. Proto je při používání SSH doporučeno několik bezpečnostních postupů, které snižují možnost napadení počítače nejčastěji pomocí slovníkových útoků.
\begin{description}
...
Nastavení jména, příjmení a~emailu.
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
...
\end{Verbatim} 
    \end{codeframe}
Pokud nahráváme data na server je třeba autentizace. K~účelu autentizace se často používá ssh klíč. Ten je možné vygenerovat následujícím způsobem
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
...
Vygenerovaný klíč je k~nalezení v~domovském adresáři uživatele ve~složce \it{.ssh}, která obsahuje soubory \it{id_rsa} a \it{id_rsa.pub}. id_rsa je váš privátní klíč a~id_rsa.pub je klíč veřejný. Pro použití githubu je~třeba veřejný klíč nahrát do~nastavení na~webu \it{“Account Settings” > Click “SSH Public Keys” > Click “Add another public key”}. Také je potřebné nahrát do nastavení gitu bezpečnostní token githubu. Bezpečnostní token lze nalézt na webu \it{“Account Settings” > Click “Account Admin.”} položku API token, která obsahuje jakýsi HASH, je třeba vložit do~nastavení git na počítači.
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
...
Vytvoření repozitáře je velmi jednoduché a~skládá se ze dvou kroků: Vytvoření složky a~inicializace repozitáře.
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
mkdir projekKteryZmeniSvet
cd projekKteryZmeniSvet
git init
\end{Verbatim} 
    \end{codeframe}
...
Pokud chceme použít github je třeba vytvořit repozitář na webu github. Po vyplnění názvu a~popisu repozitáře, github vypíše kroky nutné k~vytvoření repozitáře. Vypíše tedy, že je třeba vytvořit složku, inicializovat git, přidat první commit (nejčastěji soubor README), přidat adresu repozitáře na serveru github a~odeslání změn v~repozitáři.
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
git remote add origin git@github.com:uzivatelskeJmeno/projekKteryZmeniSvet.git
git push origin master
\end{Verbatim} 
    \end{codeframe}

\subsubsection{Správa repozitáře}
Každodenní správa repozitáře jednoduchého projektu je většinou o~přidávání commitů a~odesílání změn. Takže si většinou lze vystačit s~příkazy \it{add, rm, diff, commit a push}. Ale git obsahuje také spoustu jiných užitečných funkcí, které usnadní správu i~malého projektu.\\

Commit není pouze přidávání souborů, ale~i~mazání a~změny v~souborech.

Jednou z~funkcí je tag. Tag vytvoří značku za~určeným commitem ke které se lze jednoduše vrátit. Je tedy užitečný pokud chce vytvořit např. release programu. commity a~jejich kódy si lze prohlédnout příkazem \it{git log}
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
git tag v1.0 <kód commitu>
\end{Verbatim} 
    \end{codeframe}
K~tagu se lze jednoduše vrátit příkazem checkout.
    \begin{codeframe}
      \begin{Verbatim}{frame=single}
git checkout v1.0
\end{Verbatim} 
