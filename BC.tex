...
\vspace{1cm}
Tato bakalářská práce v teoretické části obsahuje posmy, jako jsou Linux, dále pak linoxové distribuce, jazyk BASH, zabezpečení operačního systému Linux a~LiveCD. Úvodní část je věnována informacím o~systému Linux a~distribuci Linux From Scratch. Dále pak obsahuje stručný úvod do jazyka BASH a~kompilaci zdrojových kódů včetně možností sestavení Linuxových distribucí. Na závěr teoretické části je popsáno zabezpečení Linuxu, hodnocení bezpečnosti Software a tvorba LiveCD.\\

V praktické části je rozbrána struktura scriptů a~funkce vytvořené v~jazyce BASH. Dále je uveden návod na sestavení distribuce LFS pomocí vytvořených scriptů. A na závěr uvedeny zdroje pomoci při chybách v~kompilaci a~možnosti zlepšení scriptů.
...
\begin{description}
 ...
 \item[ALFS] Automated Linux From Scratch - cílem projektu je vytvořit nástroje pro autmatické sestavování LFS a~BLFS
...
\nadpis{Builder}\\
Builder je série scriptů napsaných v~BASHi, které používají vývojáři derivátu distribuce Ubuntu "gNewSense" na sestavení jejich distribuce. Ke scriptům je také napsaný přehledný manuál jak je používat, který je dostupný online~\cite{Linux_Build}~\cite{BuilderNavod}.
...
\begin{description}
 ...
 \item[chap7] Obsahuje scripty pro nastavení bootscripts.
 \item[chap8] Obsahuje scripty pro poslední fázi sestavení.
 ...
 \item[chroot\_makeall] hlavní spouštěcí script pro sestavení kapitoly 6 v~chroot.
 \item[chroot\_prepare] script, který se stará o~kontrolu potřebných složek a~symbolických odkazů v~prostředí chroot.
 ...
 \item[onlyunpackscript] script pro stažení a~rozbalení balíčků.
...
Co by měl uživatel udělat před sestavení LFS pomocí automatických scriptů
\begin{itemize}
 \item několikrát úspěšně sestavit LFS
 \item přečíst znovu alespoň první 4kapitoly (je dobré číst knihu za běhu scriptů)
...
a~ještě překopírovat soubory užvatele lfs do jeho domovského adresáře
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
...
Protože pro stažení balíčků se používá program wget, který v~toolchainu, respektive při sestavování kapitoly 6, nebude dostupný, je dobré stáhnou všechny balíčky scriptem onlyunpackscript~\odkazNaKapitolu{sec:only}.\\

Nyní začne sestavení toolchainu, proto spustíme script makeall
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
sh ./scripts/makeall
\end{Verbatim} 
...
nyní je třeba spustit script start6, který vytvoří potřebné složky a~virtuální souborové systému.
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
...
nyní je třeba se chrootnout do systému, tedy se přihlásit vytvořného toolchainu
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
...
\end{Verbatim} 
    \end{codeframe}
Nyní je dobré nastavit opět potřebné proměnné v~souboru \/scripts\/chroot\_prepare

a spustit script chroot\_makeall
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
./scripts/chroot_makeall
\end{Verbatim} 
    \end{codeframe}
script je přerušen po opětovné přihlášení a~je potřeba ho spustit znovu.

\subsubsection{onlyunpackscript}\label{sec:only}
Tento script slouží pouze pro stažení, kontrolu a~rozbalení balíčků. Před jeho použitím je třeba mít vytvořený a~připojený diskový oddíl jak je popsáno \odkazNaKapitolu{sec:sestaveni}.\\

Je třeba stáhnopu scripty a~překopírovat na místo
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
cd ~/
git clone git://github.com/zajca/LFS---by-bash-scripts.git
...
\end{description}
Kombinací proměnných je tak možné dosáhnout pouze stažení všech balíčků bez čekání nebo stažení balíčků a~jejich rozbalení s~čekáním na další krok. Pokud chceme sestavovat LFS manuálně podle knihy, je dobré nastavit proměnné tak jak je uvedeno v~poli s~kódem. Balíček se stáhne rozbalí a~script čeká až uživatel balíček sestaví a~stiskne klávesu ENTER. Pokud při sestavení nastane chyba a~je balíček třeba znovu sestavit, stačí zadat libovolný vstup a~stisknout ENTER, balíček se znovu rozbalí. \b{!!!POZOR. Pokud se balíček sestavuje odděleným buildem, tedy ve složce balíček-build, je třeba tuto složku smazat manuálně!!!}
...
Tak jako u všech programů existuje mnoho možností vylepšení i~u~vytovřených scriptů pro sestavení LFS.\\

Možnosti vylepšení:
\begin{itemize}
 \item Částečná změna scriptů z~jazyka BASH na scripty programu GNU/make (tzv. Makafiles). Řídící script makeall a onlyunpackscript by se po změně na makefile staly lépe ovladatelnými, především při přerušení a~obnově práce.
