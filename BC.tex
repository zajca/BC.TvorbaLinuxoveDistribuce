...
\end{Verbatim} 
    \end{codeframe}
Podmínky je samozřejmě možno spojovat pomocí operátorů \&\& (a zároveň platí) a || (nebo platí). Operátor || má velké využití ve scriptech pro testování jestli funkce skončila úspěšně\cite{ABC_SBASH}.
    \begin{codeframe} 
      \begin{Verbatim}{frame=single}
...
\begin{description}
 ...
 \item[makeall] hlavní spouštěcí script, který spustí vše potřebné až do konce kapitoly 5.
...
\begin{description}
 ...
 \item[l10n-EN] už téměř nepoužívaný script, který měl sloužit pro lokalizaci do jiných jazyků, byl zavrhnut, protože znamenal velkém množství proměnných
 \end{description}
...
Funkce waitUser() na základě proměnné \$WAIT pozastavuje vykonávání scriptu, dokud uživatel nezmáčkne klávesu enter. Podobnou funkcí je pak fuknce waitUserAl(), která pozastaví scripty vždy.
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
waitUser (){
  if [ $WAIT == 1 ]; then
...
\end{Verbatim} 
    \end{codeframe}
nyní je dobré stáhnout a~překopírovat scripty LFS by bash scripts. Nejjednodušším způsobem je stáhnutí skriptů pomocí verzovacího systému git.
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
cd ~/
...
nyní je třeba se chrootnout do systému, tedy se přihlásit vytvořeného toolchainu
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
...
Je třeba stáhnout scripty a~překopírovat na místo
    \begin{codeframe} 
\begin{Verbatim}{frame=single} 
cd ~/
git clone git://github.com/zajca/LFS---by-bash-scripts.git
...
V úvodní části bylo provedeno seznámení s~Linuxem jeho distribucemi a~možnostmi tvorby distribucí.
%********************************* Seznam Literatury ************************************
\seznamlit{
